...
def test_long_line_retains_all_text_when_duplicate_history_is_trimmed(count):
    runs = [make_run("z", i * 5) for i in range(count)]
...
def test_page_footer_suppression_respects_font_size(size, fragmented):
    runs = (
        [make_run("page", size=size), make_run("1", 5, size=size)]
        if fragmented
        else [make_run("page1", size=size)]
    )
...
def test_rotated_cells_use_their_reading_axis_for_spacing(rotation, gap):
    runs = [make_run("ab", i * (5 + gap), rotation=rotation) for i in range(8)]
    if rotation == 270:
        runs.reverse()
...
def test_empty_rotated_table_text_has_no_segments(text):
    result = reconstruct_rotated_table_line([make_run(text, rotation=90)])
    assert result.text == ""
    assert result.segments == ()
