...
stages are the seams ``extract.pipeline`` itself uses, in pipeline order:
    ...
    capture_page -> extract_tables -> layout_blocks_with_evidence
...
Every fixture exercises native PDF text extraction.
"""
...
def internal_stage(page: Any) -> Staged:
    ...
    return Staged(page, capture, observations, obstacles)
