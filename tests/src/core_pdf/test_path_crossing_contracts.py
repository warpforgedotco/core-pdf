...
def test_scanline_crossings_preserve_half_open_bounds_and_edge_order(
    ...
    assert paths.fill_path_sample_crossings_numpy(array, np.array(ys, dtype=np.float64)) == expected
    assert [paths.fill_path_sample_crossings_row(array, y) for y in ys] == expected
    assert [paths.fill_path_sample_crossings(edges, y) for y in ys] == expected
...
def test_empty_crossing_inputs_preserve_row_cardinality(monkeypatch, limit, rows, edges):
    monkeypatch.setattr(paths, "INTERNAL_CROSSING_MASK_CELL_LIMIT", limit)
    result = paths.fill_path_sample_crossings_numpy(
...
def test_diagonal_crossings_interpolate_positions_and_winding(monkeypatch, limit):
    ...
    assert paths.fill_path_sample_crossings_numpy(edges, np.array([0, 1, 2, 3, 4])) == [
        [(0, 1), (4, -1)],
        [(1, 1), (3, -1)],
        [(2, 1), (2, -1)],
        [(3, 1), (1, -1)],
