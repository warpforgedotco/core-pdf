...
from core_pdf.impl._impl.capture.tolerant_state import (
    FontCompanionsCache,
    internal_font_companions,
    internal_font_signature,
)
...
def document():
    ...
    instance = SimpleNamespace(
        resolver=resolver, resolve=resolver.resolve, internal_font_decoders={}
    )
    yield instance
    resolver.close()


def internal_select(state, resources, name="F"):
...
def test_companion_groups_ignore_subset_tags_and_sort_indirect_identities(document, reverse):
    ...
    grouped = internal_font_companions(fonts, document.resolve, cache)
    assert grouped == {"Helvetica": ((1, 0), (2, 0), (3, 4))}
    assert cache[id(fonts)][0] is fonts
    assert internal_font_companions(fonts, document.resolve, cache) is grouped
...
def test_unresolved_or_unnamed_siblings_do_not_create_companion_groups(document, font):
    ...
    assert internal_font_companions({"F": PdfReference(7)}, document.resolve, {}) == {}
...
def test_nonindirect_or_malformed_resources_disable_document_sharing(document, resources, font):
    assert internal_font_signature(PdfReference(1), font, resources, document.resolve, {}) is None


def test_unnamed_font_signature_retains_reference_without_companions(document):
    signature = internal_font_signature(PdfReference(8, 2), {}, {"Font": {}}, document.resolve, {})
...
def test_separate_captures_share_font_decoders_for_equivalent_sibling_sets(
    ...
    decoder = internal_select(first, first_resources)
    assert internal_select(second, second_resources) is decoder
    assert second.graphics.decoder_resources is second_resources
    assert len(document.internal_font_decoders) == 1
...
def test_changed_or_direct_siblings_prevent_decoder_reuse_across_captures(document, sibling):
    first, second = TextState(document), TextState(document)
    original = internal_select(first, {"Font": {"F": PdfReference(1), "Sibling": PdfReference(2)}})
    changed = internal_select(second, {"Font": {"F": PdfReference(1), "Sibling": sibling}})
...
def test_alias_selection_reuses_the_capture_owned_decoder(document):
    ...
    decoder = internal_select(state, resources)
    assert state.get_decoder() is decoder
    assert internal_select(state, resources, "Alias") is decoder
    assert internal_select(state, resources, "F") is decoder
...
def test_direct_fonts_reuse_within_scope_but_never_across_captures(document, wrapped):
    ...
    first, second = TextState(document), TextState(document)
    decoder = internal_select(first, resources)
    assert internal_select(first, resources, "Alias") is decoder
    assert internal_select(second, resources) is not decoder
    assert document.internal_font_decoders == {}
...
def test_invalid_font_objects_fall_back_to_a_usable_decoder(document, value):
    ...
    state = TextState(document)
    decoder = internal_select(state, {"Font": {"F": PdfReference(9)}})
