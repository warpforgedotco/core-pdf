...
class ToUnicodeCMap(PdfToUnicodeCMap):
    ...
    def parse_bfchar_block(self, block: CMapMappingBlock) -> None:
    ...
    def parse_bfrange_block(self, block: CMapMappingBlock) -> tuple[int, int]:
        ...
        for record in block.records():
            ...
            try:
                source_range = cmap_source_range(
                    decode_cmap_hex_token(t1), decode_cmap_hex_token(t2)
                )
            ...
            if t3.startswith(b"["):
                ...
                for offset, dst_tok in enumerate(dsts):
                    ...
                    try:
                        ...
                        continue
                    self.mappings[source_range.source_at(offset)] = dst
                    added = True
                if added:
                    valid_range_count += 1
                else:
    ...
    def parse_cidrange_block(self, block: CMapMappingBlock) -> None:
        ...
        for record in block.records():
            ...
            for offset in range(source_range.count):
                self.mappings[source_range.source_at(offset)] = unicode_scalar_or_replacement(
                    destination + offset
                )
