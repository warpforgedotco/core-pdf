...
from core_pdf.impl.spec.s_07_syntax.inherited_values import (
    collect_inherited_values as collect_spec_inherited_values,
)
from core_pdf.impl.spec.s_07_syntax.types import InheritedValueMap, PdfDict, PdfValueResolver
...
def collect_inherited_values(
    node: PdfDict, keys: tuple[str, ...], resolve_ref: Callable[[object], object]
) -> InheritedValueMap:
    return collect_spec_inherited_values(node, keys, resolve_ref, on_invalid=lambda message: None)
