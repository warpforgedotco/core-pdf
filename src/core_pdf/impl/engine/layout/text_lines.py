...
class GlyphLineBuilder:
    ...
    def __init__(
        ...
        *,
        ...
        if self.has_explicit_spaces:
            self._prepare_explicit_space_context()
    ...
    def _prepare_explicit_space_context(self) -> None:
        runs = self.runs
    ...
    def normalized_text(self, run: TextRun, index: int) -> str:
        ...
        if self.is_trademark_marker_run(run, index):
            return "™"
        if self.is_superscript_like_numeric_run(run, index):
            return text.translate(SUPERSCRIPT_DIGIT_TRANSLATION)
        if self.is_subscript_like_numeric_run(run, index):
    ...
    def atom_separator(
        ...
        prev_x0, _prev_y0, prev_x1, _prev_y1 = previous.advance_bbox
        x0, y0, _x1, y1 = atom.advance_bbox
        height = y1 - y0
        x_gap = x0 - prev_x1
        spacing_gap = x_gap
    ...
    def should_join_word_fragments(
        ...
        *,
        ...
        if self.is_table_like_line or self.is_tracked_glyph_line:
            return False
...
def runs_are_left_to_right(runs: list[TextRun]) -> bool:
    ...
    prev_x0 = previous_coords[TextRun.X0]
    prev_x1 = previous_coords[TextRun.X1]
