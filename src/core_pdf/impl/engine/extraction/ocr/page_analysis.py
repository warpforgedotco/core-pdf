...
def rendered_page_for_ocr_analysis(page: PageExtractionHost) -> RenderedPage:
    ...
    rendered = cast(
        RenderedPage,
        page.render(RenderOptions(include_annotations=False, include_layers=False)),
    )
    if cache is not None:
...
def native_text_geometry_lines(
    ...
    *,
    ...
    for line in LayoutAnalyzer.cluster_into_lines(runs):
...
def has_uninterpretable_type3_fonts(page: PageExtractionHost) -> bool:
    fonts = lookup_dict_key(page.resources, "Font")
    if not isinstance(fonts, dict):
        return False
    resolver = page.document.resolver
    for font_ref in fonts.values():
        font = resolver.resolve(font_ref)
        if isinstance(font, PdfStream):
            font = font.dictionary
        if not isinstance(font, dict):
            continue
        if normalize_pdf_name(lookup_dict_key(font, "Subtype")) != "Type3":
            continue
...
def native_text_layer_has_substantial_page_coverage(
    page: PageExtractionHost, text_tokens: int
) -> bool:
    if text_tokens < 430:
        return False
    ...
    media_box = page_geometry.rect_box_tuple(getattr(page, "media_box", None))
...
def native_text_layer_has_sparse_page_coverage(page: PageExtractionHost) -> bool:
    if getattr(page, "rotation", 0) % 360 != 0:
        return False
...
def has_dominant_page_image(page: PageExtractionHost) -> bool:
    rendered = rendered_page_for_ocr_analysis(page)
...
def page_has_large_embedded_image(page: PageExtractionHost) -> bool:
    rendered = rendered_page_for_ocr_analysis(page)
    page_area = max(1.0, float(rendered.width) * float(rendered.height))
    for item in rendered.display_list.items:
        if item.kind not in {"image", "inline-image"}:
...
def figure_ocr_regions(page: PageExtractionHost) -> tuple[FigureOcrRegion, ...]:
    if getattr(page, "rotation", 0) % 360 != 0:
        return ()
    cache = getattr(page, "extraction_cache", None)
    ...
    for item_index, item in enumerate(rendered.display_list.items):
        ...
        normalized_box = page_geometry.normalize_rect((x0, y0, x1, y1))
    ...
    if len(regions) > OCR_FIGURE_MAX_REGIONS:
        regions = sorted(
            regions,
            key=lambda region: page_geometry.rect_area(region.bbox),
            reverse=True,
...
def text_geometry_overlap_ratio(
    ...
    area = page_geometry.rect_area(box)
    if area <= 0.0:
        return 1.0
    overlap = 0.0
    for line in lines:
        if line.observation.bbox is None:
            continue
        overlap += page_geometry.rect_intersection_area(line.observation.bbox, box)
    return overlap / area


def page_has_figure_ocr_region(page: PageExtractionHost) -> bool:
...
def has_meaningful_type3_encoding(font: PdfDict) -> bool:
    encoding = lookup_dict_key(font, "Encoding")
    if not isinstance(encoding, dict):
        return False
    differences_obj = lookup_dict_key(encoding, "Differences")
    if not isinstance(differences_obj, (list, tuple)):
        return False
    try:
        differences = parse_differences(list(differences_obj), normalize_pdf_name)
    except ValueError:
        return False
