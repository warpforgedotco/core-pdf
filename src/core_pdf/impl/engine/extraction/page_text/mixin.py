...
class PageExtractionMixin(PageContentMixin):
    ...
    def extract_text(self: PageExtractionHost) -> str:
        ...
        if ocr_enabled and (
            ...
            try:
                ...
                if (
                    ...
                    and ocr_postprocess.should_try_figure_ocr_supplement(self, text)
                    and ocr_postprocess.should_try_ocr_supplement(self, text)
                    and page_has_image_only_full_page_figure_region(self)
                ):
                    ocr_result = extract_ocr_page_result(
                        self,
                        ocr_session=ocr_session,
                    ...
                    if ocr_postprocess.should_reject_full_page_ocr_result(
                        ...
                        if should_replace_text_with_ocr(
                            ...
                            schematic_ocr_supplement_candidate = ocr_result.candidate
                            schematic_ocr_supplement_candidates = ocr_result.candidates
                if ocr_postprocess.should_try_figure_ocr_supplement(self, text):
        ...
        if cached_ocr_result is not None and cached_ocr_result.candidate is not None:
            geometry_repaired_output_lines = (
        ...
        direct_hyphen_repaired_text = (
            ocr_postprocess.repair_direct_hyphenated_line_continuations_text(text)
        )
