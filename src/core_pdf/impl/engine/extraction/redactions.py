...
class RedactionAnalyzer:
    ...
    def iter_text_spans(self, page: Any) -> list[RedactionTextSpan]:
        ...
        return [build_text_span(s) for s in page.get_text_spans()]

    def iter_glyphs(self, page: Any) -> list[Glyph]:
        glyphs: list[Glyph] = []
        for span in page.get_text_spans():
            seqno = span["seqno"]
            fill = span["color"]
            for code, ignored, ignored, rect in span["chars"]:
                glyphs.append(
                    Glyph(
                        text=chr(code),
    ...
    def iter_paint_spans(self, drawings: list[dict[str, Any]]) -> list[RedactionPaintSpan]:
        ...
        for drawing in drawings:
            ...
            if rect is None:
                ...
                if item_rects:
                    ...
                    for item_rect in item_rects[1:]:
                        rect = BBox.from_rect(rect).merge(BBox.from_rect(item_rect))
            if rect is None:
                continue
            ...
            spans.append(
                RedactionPaintSpan(
                    kind=span_kind,
                    bbox=BBox.from_rect(rect) if rect is not None else None,
                    seqno=drawing["seqno"],
                    fill=fill,
    ...
    def cluster_paint_spans(
        ...
        for span_group in result_groups.values():
            span_group.sort(key=lambda s: s.seqno)
            result.append(tuple(span_group))
    ...
    def build_candidate(
        ...
        if len(paint_group) > 1:
            reasons.append("grouped_paint_spans")
    ...
    def extract_redaction_features(
        ...
        return RedactionFeatures(
            ...
            has_year=has_year_token(hidden_text),
            has_date_pattern=has_date_pattern(hidden_text),
            has_filing_stamp_pattern=has_filing_stamp_pattern(hidden_text),
            is_all_caps=bool(hidden_text)
            and all(not ch.islower() for ch in hidden_text if ch.isalpha()),
            is_numeric_only=bool(hidden_text) and alpha_count == 0,
...
def has_year_token(text: str) -> bool:
    for token in text.replace("/", " ").replace("-", " ").split():
        if (
            len(token) == 4
            and token.isdigit()
            and (token.startswith("19") or token.startswith("20"))
        ):
            return True
    return False
...
def has_date_pattern(text: str) -> bool:
    ...
    return any(is_date_like(part.strip(".,;:()[]{}")) for part in parts)
...
def is_date_like(value: str) -> bool:
    ...
    if not (
        ...
        return False
    return 0 <= int(month) <= 39 and 0 <= int(day) <= 39


def has_filing_stamp_pattern(text: str) -> bool:
    folded = text.casefold()
    filed_at = folded.find("filed")
    if filed_at < 0:
        return False
    return has_date_pattern(folded[filed_at:])
