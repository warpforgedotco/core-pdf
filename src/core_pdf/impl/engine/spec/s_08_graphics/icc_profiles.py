...
def convert_icc_gray_samples(raw: bytes, profile: IccMatrixProfile) -> bytes:
    ...
    for index, value in enumerate(raw):
        ...
        gray = max(0, min(255, int(round(((red + green + blue) / 3.0) * 255.0))))
        result[index] = gray
    return bytes(result)
...
def parse_icc_matrix_profile(profile: bytes) -> IccMatrixProfile | None:
    ...
    if color_space == b"RGB " and pcs == b"XYZ ":
        ...
        if None in {
            ...
            return None
        assert red_xyz is not None
        assert green_xyz is not None
        assert blue_xyz is not None
        assert red_trc is not None
        assert green_trc is not None
        assert blue_trc is not None
        white_point = parse_icc_xyz_tag(tags.get(b"wtpt")) or (0.9642, 1.0, 0.8249)
        return IccMatrixProfile(
            color_space="RGB",
            pcs="XYZ",
...
def parse_icc_lut_profile(profile: bytes) -> IccLutProfile | None:
    ...
    if lut is None:
        return None
    input_channels = cast(int, lut["input_channels"])
    output_channels = cast(int, lut["output_channels"])
    grid_points = cast(int, lut["grid_points"])
    matrix = cast(tuple[tuple[float, float, float], ...], lut["matrix"])
    input_tables = cast(tuple[tuple[float, ...], ...], lut["input_tables"])
    clut = cast(tuple[tuple[float, ...], ...], lut["clut"])
    output_tables = cast(tuple[tuple[float, ...], ...], lut["output_tables"])
...
def parse_icc_lut_tag(payload: bytes | None) -> dict[str, object] | None:
    ...
    matrix = tuple(
        tuple(
            s15fixed16(payload[12 + (row * 3 + col) * 4 : 16 + (row * 3 + col) * 4])
            for col in range(3)
        )
        for row in range(3)
    )
