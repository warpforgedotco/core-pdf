        ...
        if font and value:
            ...
            if nameTable:
                ...
                if name:
                    ...
                    log.debug("name id %d missing from name table" % value)
        xmlWriter.newline()
...
class Char64(SimpleValue):
    ...
    def read(self, reader, font, tableDict):
        ...
        if s != tostr(data, encoding="ascii", errors="ignore"):
            log.debug('replaced non-ASCII characters in "%s"' % s)
        return s
...
class MappingEntriesConverter(BaseConverter):
    def read(self, reader, font, tableDict):
        ...
        from core_pdf.impl.third_party.fontTools.misc.iftSparseBitSet import decode as sbsDecode
