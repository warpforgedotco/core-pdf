...
- **Shape**: ICC v2.0, CMYK device space, Lab PCS. `A2B*` are `mft2` tags over a
  ...
  black point detection in `src/core_pdf/impl/graphics_icc_profiles.py` runs against; without
...
filename in `src/core_pdf/impl/graphics_device_profiles.py`, as long as it is an ICC v2
