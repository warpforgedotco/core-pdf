...
reviewed on 2026-09-12. It records implemented differences, shared algorithms, and
...
| Behavior | Version evidence | Implementation and limits |
| --- | --- | --- |
| Type 4 calculator functions | Added in PDF 1.3; ISO 32000-1/-2 §7.10.5 and PLRM third edition §3.2.2, §8.2. | **Shared implementation.** All 42 operators use PDF numeric syntax, comments, and conditional blocks; procedure objects and indirect execution are unavailable. Domain and Range clipping bracket evaluation, with exact numeric output arity. The evaluator uses finite float64 reals and signed 32-bit integers with real promotion for oversized integer literals and applicable arithmetic results. Implementation limits are 100 operand-stack entries and 255 brace levels including the outer braces. Core reuses the compiler for tints, shading, stitching children, and Alpha-mask transfers without an external runtime. `test_calculator_functions.py` covers operators, syntax, clipping, numeric rules, and limits; `test_calculator_function_reader.py` covers decoder recovery and document rendering. Sampled functions still require 8-bit samples and linear interpolation, including prescribed small-table cubic fallback; this is not complete function conformance validation. |
...
| Indexed tint palettes | ISO 32000-2 §8.6.6.3–5; Separation/DeviceN bases allowed from 1.3. | **Implemented recursive conversion.** Scalar paint and 1/2/4/8/16-bit images apply palette lookup, tint transformation, and alternate conversion in order. Decode arrays, palette rounding/clamping, source masks, ICC profiles, and rendering settings are preserved. `test_indexed_tint_reader.py` and NChannel document pixel tests cover the previously incorrect scalar/default image paths. |
| DeviceN NChannel | Added in 1.6; ISO 32000-2 §8.6.6.5, Tables 70–72, and corrected component-name rules. | **Implemented process conversion and selected spot approximation / partial mixing.** Strict parsing validates attributes, process mappings, and individual spot definitions; mixing-hint contents remain opaque. Core converts process-only spaces directly and can multiply individual spot sRGB appearances over the converted process color (white for all-spot spaces). This screen policy requires empty/absent mixing hints and white zero-tint spot endpoints. Other endpoints, nonempty hints, unusable individual functions, and malformed attributes use the global tint fallback. Tests cover scalar colors, 8/16-bit images, Indexed palettes, text, patterns, shading, ICC profiles, rendering intent, and compensation. Spectral mixing, mixing-hint rendering, and separation-aware overprinting remain unimplemented; see [architecture](architecture.md#device-colour). |
| None colorants | ISO 32000-2 §8.6.6.4–5. | **Implemented paint suppression.** Separation `None` and all-`None` DeviceN suppress raster paint, including Indexed and uncolored Pattern bases, while preserving text extraction, advances, and clipping. Fill/stroke decisions are independent; mixed `None`/real-colorant tint inputs remain intact. Strict helper tests and `test_none_colorant_paint.py` cover state, pixels, and skipped sample/function decoding. |
| Form bounding-box clipping | ISO 32000-1/2 §8.10.1–2; Form dictionary Table 95 / Table 93. | **Implemented raster scopes.** Resolved local bounds reach capture independently of raw operands and enclosing page-space bounds. Paths, shading, text, and images clip to the transformed quadrilateral, including nested and transparency Forms; zero-area bounds suppress paint. Parent clipping restores after balanced or recovered child content, while image placement and existing extraction geometry stay intact. Scope records are excluded from public drawing projections and OCR routing counts. `test_form_bbox_frames.py`, `test_form_bbox_clipping.py`, and `test_form_scope_projections.py`. |
| Form and tiling pattern transparency groups | ISO 32000-2 §11.4.4, §11.4.8, §11.6.6–7. | **Implemented isolation and non-isolated RGB composition / partial transparency.** Form isolation survives interpretation and capture, including the default non-isolated case. Non-isolated groups retain the initial backdrop and independently accumulate source alpha; backdrop removal prevents repainting that background during outer composition. All pattern tiles share one group, with the existing isolated optimization for Normal-only contents. Outer opacity, the selected soft mask, and supported blend modes apply once. Strict equations, frame/capture tests, document pixel cases, and raster alpha tests cover nesting and paint paths. Form and text knockout behavior is described below; arbitrary group blending spaces and Luminosity soft-mask rendering remain unsupported. |
| Form knockout groups and alpha source | PDF 1.4; ISO 32000-2 §11.4.6, §11.6.4, §11.6.6–7, §11.7.4.4 and Table 145. | **Implemented RGB Form composition.** `/K` defaults to false independently of `/I`. Each element blends with the initial backdrop and replaces earlier contributions by shape; nested non-isolated groups inherit that same backdrop. Paths, strokes, glyphs, images and shadings retain shape at zero opacity. Stencils and color-key masks constrain image shape; `/AIS` selects whether constants and soft masks contribute shape or opacity. Pattern alpha applies once, and overlapping parts of a stroke or combined fill/stroke avoid doubled opacity inside these groups. Strict equations, metadata/capture tests and raster/document regressions cover fractional coverage, masks and nesting. |
| Graphics-state Alpha soft masks | PDF 1.4; ISO 32000-2 §11.5, §11.6.4.3, §11.6.5–7 and Table 142. | **Implemented Alpha raster / strict Luminosity metadata.** `/SMask` retains the CTM and resource scope at `gs`, while other inherited non-transparency settings come from each paint. Separate mask capture keeps mask artwork out of page extraction. Group alpha supplies spatial coverage independently of colour/backdrop; transfer functions also apply to zero outside the Form BBox. `/None`, omitted/null entries, and `q`/`Q` preserve their distinct state semantics. Transparency groups reset inherited masks and apply the outer mask once; `/AIS` selects shape or opacity. Native image masks and embedded opacity override the graphics mask. Strict transfer compilation supports types 0/2/3/4 within the documented compiler limits, including Type 4 without an injected backend. Core leaves Luminosity unsupported, using its unmasked fallback. `test_soft_mask_state.py`, `test_alpha_soft_mask_rendering.py`, and `test_alpha_soft_mask_target.py` cover state, transforms, nesting, extraction isolation, transfer, masks and raster composition. |
| Text-object knockout and clipping | `/TK`: PDF 1.4; ISO 32000-2 §9.3.6, §9.3.8, §9.4 and §11.4.6. | **Implemented RGB text composition.** `/TK` defaults to true and groups each complete text object as a non-isolated knockout group; false lets glyphs composite individually. `/TK` entries inside `BT`/`ET` are ignored, including invalid indirect values. Glyph transparency is inherited once, and other state changes persist after `ET`. Explicit capture boundaries preserve paint sequence numbers and cover multiple show operators, nested streams and Type 3 glyph programs. Each Type 3 program is one glyph element without substitute-outline double paint. Text outlines update clipping at `ET` without restoring graphics state; surrounding scopes restore the clip. Tiling patterns retain their initial-stream `/TK`. `test_text_knockout_state.py` and `test_text_knockout_rendering.py` cover strict state, overlap, alpha source, fill/stroke, nesting, and clipping. The RGB and Luminosity soft-mask limits above still apply. |
...
| Type 3 glyph programs | ISO 32000-1 §9.3.6, §9.6.5; ISO 32000-2 §9.3.6, §9.6.4. | **Implemented execution / shared contemporary rendering-mode correction.** The interpreter executes `CharProcs` with the font/text transform, font resource scope, and glyph spacing. Capture and rendering preserve the program as one text glyph element without painting a substitute outline again. ISO 32000-1 named only mode 3; the implementation follows ISO 32000-2's explicit suppression for modes 3 and 7 across document headers. Advances remain active, and Type 3 glyphs do not contribute outlines to text clipping. `test_type3_rendering_modes.py` and `test_type3_rendering_mode_reader.py` cover all eight modes, including PDF 1.7/2.0 raster and visibility controls. |
...
[PLRM third edition](https://www.adobe.com/jp/print/postscript/pdfs/PLRM.pdf),
[syntax errata](https://pdf-issues.pdfa.org/32000-2-2020/clause07.html), and
...
UserUnit, historical encoding, current color-conversion controls, recursive Indexed tints,
None-colorant suppression, NChannel process conversion/selected spot approximation, RGB
group isolation/non-isolated composition, Form/text knockout, Alpha soft masks, and Type 4
calculator functions above are implemented.
The historical font audit distinguishes actual encoding changes from clarified or
implementation-specific mapping rules. Type 3 glyph programs execute with the shared
contemporary rendering-mode correction described above. Further chapter work includes NChannel
mixing hints and plate-aware output, halftones, arbitrary transparency-group blending spaces,
Luminosity soft-mask rendering, additional sampled-function depths and cubic interpolation,
and the remaining partial/gap rows.
