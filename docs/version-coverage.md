...
| DeviceN NChannel | Added in 1.6; ISO 32000-2 §8.6.6.5, Tables 70–72, and corrected component-name rules. | **Implemented process-only conversion / partial mixing.** Strict parsing validates attributes, process mappings, and individual spot definitions; mixing-hint contents remain opaque. Core preserves attributes and converts process-only NChannel through its process space, including additive RGB, reordered/subset CMYK, and ICC profiles. Scalar colors and 8/16-bit images retain rendering intent and compensation; document pixel tests cover paths, images, and patterns. Ordinary DeviceN, malformed attributes, and mixed spot/process spaces retain their existing fallback paths, including the preexisting scalar Indexed-to-DeviceN gap described in [architecture](architecture.md#device-colour). Individual spot mixing, mixing-hint rendering, and separation-aware overprinting remain unimplemented. |
...
UserUnit, historical encoding, current color-conversion controls, and NChannel process-only
conversion above are implemented. The historical font
audit distinguishes actual encoding changes from clarified or implementation-specific
mapping rules. Further chapter work includes NChannel spot-ink mixing, halftones, arbitrary
