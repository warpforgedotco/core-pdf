...
| Tiling pattern transparency | ISO 32000-2 §11.6.7, Notes 1–2. | **Implemented Normal-content optimization / partial groups.** All tiles share one buffer when captured contents use only Normal blending; outer opacity, scalar soft mask, and blend mode apply once to the result. Pixel tests cover overlapping marks/tiles, clipping, outer blend modes, and unchanged backdrop-dependent internal blends. Patterns with non-Normal contents still need non-isolated group support for complete outer transparency. |
...
None-colorant suppression, and NChannel process conversion/selected spot approximation above
