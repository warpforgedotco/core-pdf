...
| PDFDocEncoding Euro slot | Adobe 1.3 Appendix D.1, note 1 introduces Euro at octal 240, previously unused. | **Gap.** The shared table uses the modern assignment. An earlier undefined-code policy needs explicit treatment; do not invent a replacement glyph. |
...
| Page tree, fields, labels, metadata, annotation appearance selection | ISO 32000-2 §7.7.3, §12.5–12.7, §12.4.2, §14.3. | **Partial.** The existing `s_07_document` helpers share current rules; these chapters have not received a complete historical audit. Page `/UserUnit` (PDF 1.6) is not applied to core page geometry. |
...
| Black-point compensation and halftone origin | PDF 2.0, ISO 32000-2 §8.6.5.9 and Table 58. | **Gap.** `UseBlackPtComp` and `HTO` are not handled. |
| Standard 14 font dictionaries | Earlier dictionary omissions limited to PDF 1.0–1.7; ISO 32000-2 §9.6.2.2. | **Shared / partial.** Standard metrics remain available to readers. Checking omitted dictionary fields is a separate strict validation concern; removing metrics in 2.0 would be incorrect. |
| TrueType character mapping | Detailed mapping procedure dated to PDF 1.3 in ISO 32000-2 §9.6.6.4. | **Partial.** Core uses fontTools selection/recovery; spec owns symbolic-code helpers. Earlier mapping needs further audit before introducing a branch. |
| Composite and OpenType fonts | Type 0/CID from 1.2; OpenType from 1.6. ISO 32000-2 §9.7, §9.9. | **Shared / partial.** CMap, CFF, and width algorithms have tests. Earlier multi-descendant composite fonts require review; current dictionary helpers require one descendant. |
...
The lexical, structure-role, 16-bit image, blend, and positive-validator-fixture
work above is implemented. Further chapter audits should address the remaining
rows individually: page UserUnit, historical PDFDocEncoding and font mapping,
DeviceN NChannel, and PDF 2.0 graphics state remain separate tasks.
