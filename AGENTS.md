...
- `impl/render/` rasterizes; `impl/output.py` defines structured output and `impl/serialize.py` emits markdown/HTML/JSON. `impl/records.py` holds capture records, `impl/model/` owns lower-level geometry/text models, and `impl/layout/` owns the heuristics that consume them. `impl/runtime/` holds engine-independent infrastructure and must not import from `impl/spec/` or the derived-processing packages beside it.
