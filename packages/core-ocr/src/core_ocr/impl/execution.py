...
OcrComponentBoxCache = MutableMapping[Any, object]
...
def crop_ocr_image_region(
    ...
    if image.bytes_per_pixel not in {1, 3, 4} or not image.data:
        return None
    clamped = get_candidate_services().clamp_ocr_bbox(*rectangle, image.width, image.height)
    if clamped is None:
        return None
    ...
    return OcrImage(
        ...
        resolution=image.resolution,
        page_bbox=get_candidate_services().page_geometry.image_bbox_to_page_bbox(clamped, geometry),
