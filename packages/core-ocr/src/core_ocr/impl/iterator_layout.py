...
from core_ocr.impl.layout import OcrLayoutWordRole
...
def iterator_row_area(row: OcrRow) -> float:
    ...
    return get_candidate_services().page_geometry.rect_area(bbox) if bbox is not None else 0.0
...
def iterator_row_overlap_ratio(
    ...
    overlap = get_candidate_services().page_geometry.rect_intersection_area(left, right)
    smaller = min(
        get_candidate_services().page_geometry.rect_area(left),
        get_candidate_services().page_geometry.rect_area(right),
    )
    if smaller <= 0.0:
        return 0.0
...
def filter_unsupported_low_confidence_tile_lines(
    ...
    if len(support_tokens) < get_candidate_services().dense_vector_render_tile_min_tokens // 2:
        return layout, 0
    kept: list[OcrRow] = []
    removed = 0
...
def iterator_layout_text_result(
    ...
    *,
    ...
    if min_confidence <= 0 and layout.text.strip():
        return get_candidate_services().layout.geometry_rendered_ocr_result(
            OcrTextResult(
                layout.text,
                layout.confidence,
...
def iterator_word_row_roles(
    ...
    words = [
        ...
        if (word := get_candidate_services().layout.ocr_layout_word(row, row_index=row_index))
        is not None
    ]
    if not words:
        return {}
    return get_candidate_services().layout.ocr_rendered_layout(words).roles
...
def best_reconciled_line_alternative(
    ...
    word_roles: dict[int, OcrLayoutWordRole] | None = None,
...
def line_alternative_from_word_rows(
    ...
    if not words:
        return None
    text = get_candidate_services().layout.render_ocr_word_line(words, word_roles)
    if not text:
        return None
...
def line_alternative_from_symbol_choices_with_word_rows(
    ...
    for local_index, word_row in enumerate(word_rows):
        ...
        word = get_candidate_services().layout.ocr_layout_word(
