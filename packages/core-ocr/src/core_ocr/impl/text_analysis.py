...
def vector_text_supports_schematic_tiled_ocr(vector_text: str) -> bool:
    if extracted_text_token_count(vector_text) < OCR_SCHEMATIC_VECTOR_RENDER_TILE_MIN_TOKENS:
        return False
...
def schematic_tiled_ocr_candidate_support_metrics(
    ...
    if (
        extracted_text_token_count(support_text) < OCR_SCHEMATIC_VECTOR_RENDER_TILE_MIN_TOKENS
