...
if TYPE_CHECKING:
    ...
    from core_ocr.impl.coordinator import PageExtractionHost
...
def ocr_render_dpi_candidates_for_page(
    ...
    rendering = get_candidate_services().rendering
    candidates = rendering.ocr_render_dpi_candidates_for_vector_text(vector_text)
    dimensions = rendering.ocr_page_dimensions_points(page)
    ...
    for dpi in candidates:
        if should_prefer_tiled_rendered_ocr(
            ...
            continue
        safe_dpi = rendering.safe_ocr_render_dpi(width_points, height_points, dpi)
...
def ocr_tiled_render_dpi_candidates_for_page(
    ...
    return tuple(
        dpi
        for dpi in candidates
        if not rendering.ocr_render_pixel_size_is_supported(width_points, height_points, dpi)
...
def should_prefer_tiled_rendered_ocr(
    ...
    if not vector_text_supports_tiled_rendered_ocr(vector_text):
        return False
    rendering = get_candidate_services().rendering
    width, height = rendering.ocr_render_pixel_dimensions(width_points, height_points, dpi)
    return width * height >= rendering.OCR_DENSE_VECTOR_RENDER_TILE_MIN_PIXELS
...
def vector_text_supports_tiled_rendered_ocr(vector_text: str) -> bool:
    ...
    if vector_tokens >= get_candidate_services().rendering.OCR_DENSE_VECTOR_RENDER_TILE_MIN_TOKENS:
        return True
...
def ocr_render_tile_max_side_pixels_for_page(
    ...
    dimensions = get_candidate_services().rendering.ocr_page_dimensions_points(page)
    if dimensions is None:
        return get_candidate_services().rendering.OCR_RENDER_TILE_MAX_SIDE_PIXELS
    width_points, height_points = dimensions
    if should_prefer_tiled_rendered_ocr(width_points, height_points, dpi, vector_text):
        return get_candidate_services().rendering.OCR_DENSE_VECTOR_RENDER_TILE_MAX_SIDE_PIXELS
