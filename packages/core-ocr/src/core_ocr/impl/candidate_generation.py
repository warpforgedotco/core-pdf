...
def should_try_line_art_text_mask_ocr_candidate(
    ...
    if confidence is not None and confidence >= 88 and text_ocr_quality_score(text) <= 0.12:
        return False
    return not (
        get_candidate_services().selection.ocr_candidate_score(base_candidate) >= 78.0
...
def should_keep_rendered_sparse_page_ocr_candidate(
    ...
    if sparse_confidence >= base_confidence - 8 and sparse_quality + 0.04 < base_quality:
        return True
    return (
        get_candidate_services().selection.ocr_candidate_score(sparse_candidate)
        >= get_candidate_services().selection.ocr_candidate_score(base_candidate) - 4.0
    )
...
def low_confidence_word_refinement_ocr_candidate(
    ...
    *,
    ...
    symbol_rows_by_line = get_candidate_services().iterator_layout.iterator_rows_by_line_key(
        symbol_rows
    )
    refinements = (
    ...
    for word_index, row in enumerate(word_rows):
        if not low_confidence_word_row_should_be_refined(row):
            continue
        line_key = get_candidate_services().iterator_layout.iterator_line_key(row)
    ...
    if not changed_word_indexes:
        return None
    changed_lines = {
        get_candidate_services().iterator_layout.iterator_line_key(word_rows[index])
        for index in changed_word_indexes
    }
    word_rows_by_line = get_candidate_services().iterator_layout.iterator_rows_by_line_key(
        tuple(word_rows)
    )
    refined_line_rows: list[dict[str, Any]] = []
    for line_row in line_rows:
        line_key = get_candidate_services().iterator_layout.iterator_line_key(line_row)
        ...
        if not text:
            refined_line_rows.append(line_row)
            continue
        row = get_candidate_services().iterator_layout.iterator_line_row_with_text(
            line_row,
            text,
            line_word_rows,
        )
        confidence = get_candidate_services().iterator_layout.iterator_rows_confidence(
            line_word_rows
        )
        if confidence is not None:
            row["conf"] = confidence
        refined_line_rows.append(row)
    result = get_candidate_services().iterator_layout.iterator_rows_text_result(refined_line_rows)
    refined_result = get_candidate_services().layout.geometry_rendered_ocr_result(
        OcrTextResult(
            result.text,
            result.confidence,
...
def batched_low_confidence_word_refinements(
    ...
    *,
    ...
    for word_index, row in enumerate(word_rows):
        ...
        base_confidence = get_candidate_services().iterator_layout.iterator_row_confidence(row) or 0
        ...
        for pad in OCR_LOW_CONFIDENCE_WORD_REFINEMENT_PADS:
            ...
            if rectangle is None:
                continue
            requests.extend(
                get_candidate_services().rectangle_request_type(
                    rectangle=rectangle,
                    psm=psm,
                    variables={},
                )
...
def low_confidence_word_row_should_be_refined(row: dict[str, Any]) -> bool:
    confidence = get_candidate_services().iterator_layout.iterator_row_confidence(row)
...
def crop_word_refinement_image(
    ...
    if rectangle is None:
        return None
    return get_candidate_services().crop_ocr_image_region(
        image,
        rectangle,
    )
...
def high_confidence_symbol_agreement_score(
    ...
    for index, row in enumerate(
        ...
        confidence = get_candidate_services().iterator_layout.iterator_row_confidence(row) or 0
...
def rebuild_ocr_line_text_from_word_rows(
    ...
    for row_index, row in enumerate(rows):
        word = get_candidate_services().layout.ocr_layout_word(row, row_index=row_index)
        if word is not None:
            words.append(word)
    if not words:
        return ""
    return get_candidate_services().layout.render_ocr_word_line(
...
def ocr_candidate_from_image(
    ...
    *,
    ...
    geometry = get_candidate_services().page_geometry.ImageSpace.from_ocr_image(image, source=name)
...
def ocr_result_with_page_observations(
    ...
    *,
    ...
    geometry = get_candidate_services().page_geometry.ImageSpace.from_dimensions(
    ...
    line_rows = tuple(
        get_candidate_services().page_geometry.normalize_ocr_row_to_page(
            row,
            geometry,
            token_type_classifier=token_type_classifier,
        )
    ...
    if row_observations:
        observations = tuple(
            get_candidate_services().page_geometry.annotate_ocr_observation_page_geometry(
                observation,
                geometry,
                source=source,
            )
        ...
        observations = tuple(
            get_candidate_services().page_geometry.normalize_ocr_observation_to_page(
                observation,
                geometry,
                source=source,
                token_type_classifier=token_type_classifier,
    ...
    return get_candidate_services().layout.geometry_rendered_ocr_result(normalized)
...
def stable_encoded_full_page_image_ocr_candidate(
    ...
    *,
    ...
    if quality > 0.30:
        return False
    return get_candidate_services().selection.ocr_candidate_score(candidate) >= 95.0
