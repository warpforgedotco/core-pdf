...
POSITIVES = Path(__file__).parent / "fixtures/positive"
MANIFEST = json.loads((POSITIVES / "manifest.json").read_text())
POSITIVE_CASES: tuple[tuple[str, str, str], ...] = tuple(
...
def test_positive_fixture_provenance_and_complete_profile_coverage() -> None:
    profiles = [profile for profile, filename, sha256 in POSITIVE_CASES]
    assert len(profiles) == len(set(profiles))
    assert set(profiles) == {support.identifier for support in VeraPdfBackend.supported_profiles}
    for fixture in MANIFEST["fixtures"]:
        data = (POSITIVES / fixture["filename"]).read_bytes()
    ...
    source = (POSITIVES / "pdfa-2a-actualtext.pdf").read_bytes()
    derived = (POSITIVES / "pdfa-3a-actualtext.pdf").read_bytes()
    assert source.count(b'pdfaid:part="2"') == 1
    assert derived == source.replace(b'pdfaid:part="2"', b'pdfaid:part="3"')


@pytest.mark.parametrize("profile", [profile for profile, filename, sha256 in POSITIVE_CASES])
def test_recorded_real_pass_for_every_advertised_profile(profile: str) -> None:
    raw = (POSITIVES / f"verapdf-1.30.2-{profile}-pass.xml").read_bytes()
    result = parse_report(raw, profile=profile, returncode=0, stderr=b"")
...
@pytest.mark.parametrize(("profile", "filename", "sha256"), POSITIVE_CASES)
...
def test_installed_engine_passes_all_three_original_accessibility_declarations(
    ...
    source = POSITIVES / "pdfua-2-wtpdf-unicode.pdf"
    original = source.read_bytes()
