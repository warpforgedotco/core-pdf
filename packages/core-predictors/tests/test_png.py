...
from core_predictors import PredictorError, UnsupportedPngFilterError, png_predict
...
def test_png_rejects_an_incomplete_final_row_before_decoding_any_filter() -> None:
    with pytest.raises(PredictorError, match="^truncated PNG predictor row$"):
        png_predict(b"\x05\x11\x00", columns=1, colors=1, bits_per_component=8)


def test_png_rejects_unknown_filter_types() -> None:
...
def test_png_rejects_depths_outside_the_specification() -> None:
