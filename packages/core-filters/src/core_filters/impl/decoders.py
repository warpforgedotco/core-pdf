...
from core_filters.impl.helpers import coerce_to_bytes, is_pdf_null, lookup_dict_key
...
def decode_jpeg(data: bytes, parms: object) -> bytes:
    try:
        ...
        raise FilterParseError("invalid JPEG stream") from exc
...
def decode_jpx(data: bytes, parms: object) -> bytes:
    try:
        ...
        raise FilterParseError("invalid JPX stream") from exc
