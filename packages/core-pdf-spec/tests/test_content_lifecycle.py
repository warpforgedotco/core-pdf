...
class EventSink:
    ...
    def text_boundary(self, state: TextState, reason: str) -> None:
        if reason == "stream-end":
            self.streams.append(("end", state.xobject_depth))


def new_state() -> tuple[TextState, EventSink]:
    sink = EventSink()
    state = TextState(
        SimpleNamespace(resolver=ObjectResolver(b"", {}, {})),
...
def test_path_paints_before_installing_clip_and_retains_geometry(
    ...
    dispatch_operations(PdfLexer(b"0 0 m 10 0 l 0 10 l " + clip), state.get_operation_handler, 0)
    path = state.current_path
    assert sink.paths == []
    dispatch_operations(PdfLexer(operator), state.get_operation_handler, 0)
    ...
    dispatch_operations(PdfLexer(b"0 0 1 1 re f"), state.get_operation_handler, 0)
    assert [event[0] for event in sink.paths].count("clip") == 1


def test_empty_completion_and_direct_flush_clear_the_current_point() -> None:
    state, sink = new_state()
    state.op_paint_clear((), 0)
    assert sink.paths == []
    dispatch_operations(PdfLexer(b"1 2 m 3 4 l W"), state.get_operation_handler, 0)
    state.flush_drawing("stroke")
...
def test_pending_clip_is_path_state_and_not_a_graphics_save() -> None:
    ...
    state, sink = new_state()
    state.op_q((), 0)
    dispatch_operations(PdfLexer(b"0 0 2 2 re W*"), state.get_operation_handler, 0)
    state.op_Q((), 0)
    state.op_paint_clear((), 0)
...
def test_flatness_defaults_fractions_and_graphics_restore(flatness: float) -> None:
    ...
    dispatch_operations(PdfLexer(b"0 0 m 0 10 10 10 10 0 c"), state.get_operation_handler, 0)
...
def test_nested_failure_unwinds_every_entered_frame(
    ...
    if stage == "enter":
        ...
        def fail_enter(current: TextState, frame: ContentStreamFrame) -> None:
            enter(current, frame)
            if frame.depth == 1:
                raise error_type("child failed")
        ...
        def fail_boundary(current: TextState, reason: str) -> None:
