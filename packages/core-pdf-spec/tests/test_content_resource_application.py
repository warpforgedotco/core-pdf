...
def internal_state() -> TextState:
    return TextState(
        SimpleNamespace(resolver=ObjectResolver(b"", {}, {})), cast(Any, None), cast(Any, None)
    )
...
def test_extgstate_updates_only_present_fields_and_clamps_opacity(entry_point: str) -> None:
    ...
    assert (state.fill_opacity, state.stroke_opacity, state.blend_mode) == (0.0, 1.0, "Multiply")
    state.apply_extgstate({"ca": None, "CA": None, "BM": []})
    assert (state.fill_opacity, state.stroke_opacity, state.blend_mode) == (0.0, 1.0, "Multiply")
    state.apply_extgstate({"CA": 0.25, "BM": PdfName.of("Screen")})
    assert (state.fill_opacity, state.stroke_opacity, state.blend_mode) == (0.0, 0.25, "Screen")
