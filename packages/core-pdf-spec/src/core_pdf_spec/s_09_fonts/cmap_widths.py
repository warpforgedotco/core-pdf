...
def parse_cid_widths(value: Any) -> FontWidthMap:
    ...
    if len(value) == 2 and type(value[0]) is int and isinstance(value[1], (list, tuple)):
        ...
        if (
            0 <= first <= MAX_CID
            and first + len(entries) <= (MAX_CID + 1)
            and all(type(item) in {int, float} for item in entries)
        ):
            return CompactCIDWidthMap(
                first, tuple(parse_float_strict(entry, "invalid CID width") for entry in entries)
            )
    widths: dict[int, float] = {}
    index = 0
    while index < len(value):
        first = value[index]
        if type(first) is not int or not 0 <= first <= MAX_CID or index + 1 >= len(value):
            raise ValueError("invalid CID widths array")
        item = value[index + 1]
        if isinstance(item, (list, tuple)):
            ...
            if type(item) is not int or not first <= item <= MAX_CID or index + 2 >= len(value):
                raise ValueError("invalid CID width range")
