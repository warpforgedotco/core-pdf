...
| `shading_values`, `shading_blend` | nothing deleted: `core_pdf.impl.render.target.paint_shading`'s per-pixel loop -- `axial_shading_t`/`radial_shading_t`, the extend clamps and `blend_px` -- for normal blending with at most a source alpha plane, which falls back to the loop otherwise; the colour of each distinct value stays in Python. Tested against the retained functions and against the loop, end to end | -30% on PyMuPDF test_4466 render (0.82 to 0.58 s); every corpus shading paints the same |
