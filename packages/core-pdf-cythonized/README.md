...
| `fill_rect_coverage` | the blend and group-plane records of `fill_rect`'s partially covered rectangles, and their two coverage-plane calls (deleted; the coverage plane was first `rect_coverage_plane`, 8.57x over the numpy of `core_pdf.impl.render.paths`, -21% on a vector-heavy page render, and is now pinned by its golden vectors through this kernel) | five kernel round trips per rectangle become one pass; -5% render on final-r3-report-emergency p4, test_2533, chinese-tables |
...
| `interleave_soft_mask` | the gather and two strided copies of `core_pdf.impl.graphics.images.apply_soft_mask` (deleted) | 10x on a 12-megapixel image with a half-size mask (128 to 13 ms) |
...
| `composite_elementary_knockout` | `core_pdf.impl.render.target.composite_group`'s element copy, its two kernel calls and the numpy shape update for an opaque normal elementary group in a knockout parent (deleted for that case) | one pass instead of three kernels and three array operations per glyph in a text knockout group; -8% on lyft text-page render |
| `fill_glyph_knockout` | the backdrop copy, the two zeroed float32 planes and the `fill_glyph_coverage` + `composite_elementary_knockout` pair in `core_pdf.impl.render.target.knockout_glyph_fill` (deleted); both kernels stay, and the composition of them is the reference it is tested against | three scratch arrays and two passes per glyph of a text knockout group become one pass; -4% on lyft text-page render |
| `shading_values`, `shading_blend`, `shading_t` | `core_pdf.impl.render.target.paint_shading`'s per-pixel loop and `core_pdf.impl.render.patterns.axial_shading_t`/`radial_shading_t` (all deleted): the parameter, the extend clamps, and `blend_px`'s blending and plane records for every mode it knows, including where the loop failed part way; the colour of each distinct value stays in Python. `shading_t` is pinned to the deleted functions by golden vectors | -30% on PyMuPDF test_4466 render (0.82 to 0.58 s); every corpus shading paints the same |
