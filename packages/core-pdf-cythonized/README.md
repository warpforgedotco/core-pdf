...
| `translated_outline_edges` | the translation and the four numpy bound reductions around `outline_edges` in `core_pdf.impl.render.commands.transformed_outline` (deleted) | six array operations per drawn glyph become one call; -5% on ISO 32000-2 text-page render |
