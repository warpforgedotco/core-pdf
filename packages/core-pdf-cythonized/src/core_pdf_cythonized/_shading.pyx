...
shading_values is the per-pixel parameter, and shading_blend the blend, for
normal blending with at most a source alpha plane to record into; the colour of
each distinct value stays in Python, where the shading's function is. Every
...
from core_pdf_cythonized._pixel_blend cimport blend_normal_pixel, plane_accumulate

__all__ = ("shading_blend", "shading_values")
...
def shading_blend(
    ...
    const int[:, ::1] colors,
    float[:, :] alpha_plane=None,
):
    """Blend each painted pixel of the box with its colour, as blend_px does in normal mode.

    ``color_index`` holds, in row-major order of the painted pixels, the row
    of ``colors`` (r, g, b, a) each is painted with. With ``alpha_plane``,
    the group's source alpha plane over the whole buffer, each pixel of
    non-zero alpha is first recorded into it as record_plane records one
    pixel. Returns (x0, y0, x1, y1), half-open, of the pixels that extend
    the paint window -- every painted one without a plane, as blend_px
    extends it before it looks at alpha; with one, those record_plane
    records -- or None.
    """
    ...
    cdef bint has_plane = alpha_plane is not None
    if has_plane and (
    ...
    cdef Py_ssize_t r, col, k = 0, count = color_index.shape[0]
    ...
    with nogil:
        for r in range(height):
            for col in range(width):
                ...
                if k >= count:
                    with gil:
                        raise ValueError("fewer colour indexes than painted pixels")
                which = color_index[k]
                k += 1
                ...
                if sa <= 0:
                    continue
                if has_plane:
                    alpha_plane[iy0 + r, ix0 + col] = plane_accumulate(
                        alpha_plane[iy0 + r, ix0 + col], sa / 255.0
                    )
                blend_normal_pixel(
                    &pixels[iy0 + r, ix0 + col, 0],
                    colors[which, 0],
                    colors[which, 1],
                    colors[which, 2],
                    sa,
    ...
    if k != count:
        raise ValueError("more colour indexes than painted pixels")
    if high_x < 0:
        return None
    return ix0 + low_x, iy0 + low_y, ix0 + high_x + 1, iy0 + high_y + 1
