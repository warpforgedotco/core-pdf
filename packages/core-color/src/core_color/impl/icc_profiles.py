...
def convert_icc_profile_samples(
    raw: bytes, profile: bytes | None, *, gray_to_rgb: bool = True
) -> bytes | None:
    if profile is None:
        return None
    ...
    if parsed.color_space == "GRAY":
        if not raw:
            return raw
        return convert_icc_gray_samples(raw, parsed, to_rgb=gray_to_rgb)
...
def convert_icc_gray_samples(
    raw: bytes, profile: IccMatrixProfile, *, to_rgb: bool = True
) -> bytes:
    result = bytearray(len(raw) * (3 if to_rgb else 1))
    for index, value in enumerate(raw):
        ...
        if to_rgb:
            ...
            result[index] = max(0, min(255, int(round(((red + green + blue) / 3.0) * 255.0))))
    return bytes(result)
