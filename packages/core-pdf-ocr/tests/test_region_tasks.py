...
def test_task_groups_preserve_order_identity_modes_and_batch_limits() -> None:
    ...
    assert region_tasks.ocr_task_groups(()) == ()
    repeated = (tasks[0],) * 17
    assert tuple(map(len, region_tasks.ocr_task_groups(repeated))) == (16, 1)
    other = replace(tasks[0], image=RasterImage(bytes(100), 10, 10, 1))
    assert tuple(map(len, region_tasks.ocr_task_groups((tasks[0], other)))) == (1, 1)
    large = replace(tasks[0], rectangle=(0, 0, 2000, 2000))
    assert tuple(map(len, region_tasks.ocr_task_groups((large,) * 3))) == (2, 1)
...
def test_binary_tiles_share_preprocessed_image_and_recognition_options() -> None:
    ...
    tasks = region_tasks.tile_tasks(source, (10, 20, 30, 120), operation, compact_image=True)
...
def test_utility_grid_maps_page_origin_and_edges_without_counting_outside_text() -> None:
    ...
    grid = region_tasks.observation_utility_grid(observations, (10, 0, 110, 100), 2, 2)
    assert grid[0] > 0
    assert grid[0] == grid[3]
    assert grid[1] == grid[2] == 0
    assert (
        region_tasks.observation_utility_grid(observations, (300, 0, 400, 100), 2, 2).tolist()
        == [0] * 4
    )
...
def test_high_resolution_rescue_deduplicates_source_modes_and_maps_crop(
    ...
    monkeypatch.setattr(region_tasks, "rendered_page_raster", render)
    result = region_tasks.high_resolution_weak_region_tasks(
        ocr_capture,
        source_tasks,
        operation,
    ...
    assert (
        region_tasks.high_resolution_weak_region_tasks(
            ocr_capture,
            (),
            operation,
            ObservationBatch.empty(),
...
def test_candidate_regions_use_direct_pixels_unless_overlaid_images_need_compositing(
    ...
    monkeypatch.setattr(region_tasks, "rendered_page_raster", render)
    result = region_tasks.candidate_region_tasks(
        ocr_capture,
        (OcrRegion(box, 1, ("test",)),),
        ocr_pass(),
        rendered=None,
        compact_image=False,
    )
...
def test_direct_scan_routing_respects_opt_out_and_native_page_evidence(
    ...
    assert region_tasks.direct_scan_allowed(capture, plan) is expected
...
def test_candidate_region_falls_back_to_dominant_scan_or_renderer(
    ...
    def fallback(capture, *, max_pixels):
        ...
        return RasterRegion(source, box) if dominant else None
    ...
    operation = ocr_pass()
    monkeypatch.setattr(region_tasks, "page_image_regions", lambda *a, **k: ())
    monkeypatch.setattr(region_tasks, "dominant_image_region", fallback)
    monkeypatch.setattr(region_tasks, "rendered_page_raster", render)
    tasks = region_tasks.candidate_region_tasks(
        ocr_capture,
        (OcrRegion(box, 1, ("test",)),),
        operation,
        rendered=None,
