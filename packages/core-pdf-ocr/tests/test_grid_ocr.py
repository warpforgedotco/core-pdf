...
def test_row_gap_closing_fills_only_bounded_gaps_up_to_limit(gap: int) -> None:
    ...
    numpy.testing.assert_array_equal(grids.internal_close_row_gaps(mask, gap), expected)
...
def test_longest_runs_do_not_cross_row_boundaries() -> None:
    ...
    assert grids.internal_longest_true_runs(mask).tolist() == [2, 4, 0]
    assert grids.internal_longest_true_runs(numpy.zeros((3, 4), dtype=bool)).tolist() == [0, 0, 0]
    assert grids.internal_cluster_line_positions(numpy.asarray([1, 2, 7, 20, 21])) == [3, 20]
    assert grids.internal_cluster_line_positions(numpy.asarray([])) == []
...
def test_detected_grid_edges_map_back_from_pooling(channels: int) -> None:
    detected = grids.internal_detect_ruling_grid(internal_image(channels))
...
def test_small_blank_and_unsupported_rasters_do_not_claim_a_grid(
    ...
    assert (
        grids.internal_detect_ruling_grid(
...
def test_skew_estimation_and_shear_straighten_rulings() -> None:
    ...
    for y in (20, 40, 60):
        dark[y, :200] = True
        dark[y + 4, -200:] = True
    assert grids.internal_estimate_ruling_skew(dark) == pytest.approx(0.01)
    assert grids.internal_estimate_ruling_skew(dark[:, :100]) == 0
    assert grids.internal_estimate_ruling_skew(numpy.zeros_like(dark)) == 0
    source = numpy.arange(12).reshape(4, 3)
    numpy.testing.assert_array_equal(
        grids.internal_vertical_shear(source, 1), [[0, 4, 8], [3, 7, 11], [6, 10, 11], [9, 10, 11]]
    )


def internal_task() -> internal_OcrTask:
    image = RasterImage(bytes([255]) * 10000, 100, 100, 1)
    return internal_OcrTask(6, image, (0, 0, 100, 100), (10, 20, 210, 220), 80)
...
def test_cell_tasks_inset_rules_skip_empty_cells_and_keep_page_transform(channels: int) -> None:
    ...
    samples[5:10, 5:10] = 0
    result = grids.internal_grid_cell_tasks(task, [0, 20, 40], [0, 20, 40], samples, 0)
    assert len(result) == 1
    cell = result[0]
    assert cell.mode == 7
    ...
    assert grids.internal_grid_region_page_box(task, [0, 20, 40], [0, 20, 40]) == (10, 140, 90, 220)
...
def test_invalid_or_excessive_cells_do_not_schedule_ocr(xs: list[int], ys: list[int]) -> None:
    assert (
        grids.internal_grid_cell_tasks(
...
def test_row_observations_merge_in_reading_order_and_average_confidence() -> None:
    ...
    result = grids.internal_grid_row_observations(source)
    ...
    assert grids.internal_grid_row_observations(empty) is empty
...
def test_gap_closing_preserves_blank_full_and_zero_width_rows() -> None:
    for mask in (
        ...
        numpy.testing.assert_array_equal(grids.internal_close_row_gaps(mask, 20), mask)
...
def test_regular_table_gate_limits_observations_crossing_column_rules(
    ...
    assert grids.internal_grid_is_regular_table(grid, prior, internal_task()) is allowed
    assert grids.internal_grid_is_regular_table(grid, ObservationBatch.empty(), internal_task())
...
def test_regular_table_gate_rejects_small_degenerate_and_irregular_grids(
    ...
    assert not grids.internal_grid_is_regular_table(grid, ObservationBatch.empty(), internal_task())
...
def test_skew_requires_three_plausibly_paired_lines() -> None:
    ...
    for y in (20, 60, 100):
        dark[y, :200] = True
        dark[y + 20, -200:] = True
    assert grids.internal_estimate_ruling_skew(dark) == 0
...
def test_grid_detection_rejects_tightly_packed_rules() -> None:
    ...
    assert grids.internal_detect_ruling_grid(RasterImage(samples.tobytes(), 300, 300, 1)) is None
